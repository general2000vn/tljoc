...
<div class="row">
    <aside class="column">
        <div class="side-nav">
            ...
            <?= $this->Html->link(__('Edit Or Status'), ['action' => 'edit', $orStatus->id], ['class' => 'side-nav-item']) ?>
            <?= $this->Form->postLink(__('Delete Or Status'), ['action' => 'delete', $orStatus->id], ['confirm' => __('Are you sure you want to delete # {0}?', $orStatus->id), 'class' => 'side-nav-item']) ?>
            <?= $this->Html->link(__('List Or Statuses'), ['action' => 'index'], ['class' => 'side-nav-item']) ?>
            <?= $this->Html->link(__('New Or Status'), ['action' => 'add'], ['class' => 'side-nav-item']) ?>
        </div>
    </aside>
    <div class="column-responsive column-80">
        <div class="orStatuses view content">
            <h3><?= h($orStatus->name) ?></h3>
            <table>
                <tr>
                    <th><?= __('Name') ?></th>
                ...
                <tr>
                    <th><?= __('Is Deleted') ?></th>
                    <td><?= $orStatus->is_deleted ? __('Yes') : __('No'); ?></td>
                </tr>
            </table>
            <div class="related">
                <h4><?= __('Related Order Reqs') ?></h4>
                <?php if (!empty($orStatus->order_reqs)) : ?>
                <div class="table-responsive">
                    <table>
                        ...
                        <?php foreach ($orStatus->order_reqs as $orderReqs) : ?>
                        <tr>
