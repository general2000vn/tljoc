...
<div class="row">
    <aside class="column">
        <div class="side-nav">
            ...
            <?= $this->Html->link(__('Edit User Title'), ['action' => 'edit', $userTitle->id], ['class' => 'side-nav-item']) ?>
            <?= $this->Form->postLink(__('Delete User Title'), ['action' => 'delete', $userTitle->id], ['confirm' => __('Are you sure you want to delete # {0}?', $userTitle->id), 'class' => 'side-nav-item']) ?>
            <?= $this->Html->link(__('List User Titles'), ['action' => 'index'], ['class' => 'side-nav-item']) ?>
            <?= $this->Html->link(__('New User Title'), ['action' => 'add'], ['class' => 'side-nav-item']) ?>
        </div>
    </aside>
    <div class="column-responsive column-80">
        <div class="userTitles view content">
            <h3><?= h($userTitle->name) ?></h3>
            <table>
                <tr>
                    <th><?= __('Name') ?></th>
                    <td><?= h($userTitle->name) ?></td>
                </tr>
                <tr>
                    <th><?= __('Id') ?></th>
                    <td><?= $this->Number->format($userTitle->id) ?></td>
                </tr>
                <tr>
                    <th><?= __('Is Deleted') ?></th>
                    <td><?= $userTitle->is_deleted ? __('Yes') : __('No'); ?></td>
                </tr>
            </table>
            <div class="related">
                <h4><?= __('Related Users') ?></h4>
                <?php if (!empty($userTitle->users)) : ?>
                <div class="table-responsive">
                    <table>
                        ...
                        <?php foreach ($userTitle->users as $users) : ?>
                        <tr>
                            <td><?= h($users->id) ?></td>
