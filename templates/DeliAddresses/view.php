...
<div class="row">
    <aside class="column">
        <div class="side-nav">
            ...
            <?= $this->Html->link(__('Edit Deli Address'), ['action' => 'edit', $deliAddress->id], ['class' => 'side-nav-item']) ?>
            <?= $this->Form->postLink(__('Delete Deli Address'), ['action' => 'delete', $deliAddress->id], ['confirm' => __('Are you sure you want to delete # {0}?', $deliAddress->id), 'class' => 'side-nav-item']) ?>
            <?= $this->Html->link(__('List Deli Addresses'), ['action' => 'index'], ['class' => 'side-nav-item']) ?>
            <?= $this->Html->link(__('New Deli Address'), ['action' => 'add'], ['class' => 'side-nav-item']) ?>
        </div>
    </aside>
    <div class="column-responsive column-80">
        <div class="deliAddresses view content">
            <h3><?= h($deliAddress->name) ?></h3>
            <table>
                <tr>
                    <th><?= __('Name') ?></th>
                ...
                <tr>
                    <th><?= __('Is Deleted') ?></th>
                    <td><?= $deliAddress->is_deleted ? __('Yes') : __('No'); ?></td>
                </tr>
            </table>
            <div class="text">
                <strong><?= __('Address') ?></strong>
                <blockquote>
                    <?= $this->Text->autoParagraph(h($deliAddress->address)); ?>
                </blockquote>
            </div>
            <div class="related">
                <h4><?= __('Related Order Reqs') ?></h4>
                <?php if (!empty($deliAddress->order_reqs)) : ?>
                <div class="table-responsive">
                    <table>
                        ...
                        <?php foreach ($deliAddress->order_reqs as $orderReqs) : ?>
                        <tr>
